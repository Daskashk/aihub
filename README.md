<div align="center">
  <img src="fastlane/metadata/android/en-US/images/icon.png" width="128" alt="">
  <h1>AI Hub</h1>
  <p><strong>All your AI assistants, beautifully together.</strong><br>Privacy-first, open-source, and completely customizable.</p>
</div>

<br>

## ✨ Features

🎨 **Modern Material You design** with dynamic colors and auto dark/light mode  
🔄 **Fully customizable interface** - reorder, hide, or disable any AI assistant  
📱 **Smooth tabbed browsing** with persistent sessions and background support  
🔒 **Privacy-focused** - blocks trackers and unnecessary connections by default  
📏 **Adjustable font size** for comfortable reading  
⚡ **Lightweight and optimized** for fast performance  
💾 **Persistent conversations** that survive app restarts  
🎯 **Clean, intuitive interface** with no distractions  

<br>

## 📸 Screenshots

<div align="center">
  <table cellspacing="12" cellpadding="0">
    <tr>
      <td><img src="fastlane/metadata/android/en-US/images/phoneScreenshots/1.png" width="240" style="border-radius:18px; box-shadow:0 6px 20px rgba(0,0,0,0.15);"></td>
      <td><img src="fastlane/metadata/android/en-US/images/phoneScreenshots/2.png" width="240" style="border-radius:18px; box-shadow:0 6px 20px rgba(0,0,0,0.15);"></td>
    </tr>
    <tr>
      <td><img src="fastlane/metadata/android/en-US/images/phoneScreenshots/3.png" width="240" style="border-radius:18px; box-shadow:0 6px 20px rgba(0,0,0,0.15);"></td>
      <td><img src="fastlane/metadata/android/en-US/images/phoneScreenshots/4.png" width="240" style="border-radius:18px; box-shadow:0 6px 20px rgba(0,0,0,0.15);"></td>
    </tr>
  </table>
</div>

<br>

## 📥 Download Now

<div align="center">

  <div style="display: flex; justify-content: center; align-items: center; flex-wrap: wrap; gap: 8px;">
    
  <a href="https://f-droid.org/packages/com.foss.aihub" style="font-size: 0;">
      <img src="https://f-droid.org/badge/get-it-on.png" width="160">
    </a>

   <a href="https://www.openapk.net/ai-hub/com.foss.aihub/" style="font-size: 0;">
      <img src="https://www.openapk.net/images/openapk-badge.png" width="160">
    </a>

   <a href="https://github.com/SilentCoderHere/aihub/releases/latest" style="font-size: 0;">
      <img src="./fastlane/metadata/android/en-US/images/badge_github.png" width="160">
    </a>
    
  </div>

  <div style="display: flex; justify-content: center; margin-top: 8px;">
    <a href="https://apps.obtainium.imranr.dev/redirect.html?r=obtainium://app/%7B%22id%22%3A%22com.foss.aihub%22%2C%22url%22%3A%22https%3A%2F%2Fgithub.com%2FSilentCoderHere%2Faihub%22%2C%22author%22%3A%22Silent%20Coder%22%2C%22name%22%3A%22AI%20Hub%22%2C%22preferredApkIndex%22%3A0%2C%22additionalSettings%22%3A%22%7B%5C%22includePrereleases%5C%22%3Afalse%2C%5C%22fallbackToOlderReleases%5C%22%3Atrue%7D%22%7D" style="font-size: 0;">
      <img src="https://raw.githubusercontent.com/ImranR98/Obtainium/main/assets/graphics/badge_obtainium.png" width="140">
    </a>
  </div>
  
  <div style="margin-top: 80px; text-align: center; font-size: 0.8em; color: #555;">
    ⚠️ F-Droid version is outdated because this <a href="https://github.com/SilentCoderHere/aihub/issues/13">issue</a> uses GitHub release to update the app until we solve this issue.
  </div>

</div>

<br>

## 🙏 Special Thanks

- **[Nora](https://github.com/nonbili/Nora)** – Gave me the original idea for this app
- **[gptAssist](https://github.com/woheller69/gptAssist)** – Inspired how to block unnecessary connections
- **[Assistral](https://github.com/shano/assistral)** – Provided necessary connections list for Mistral
- **[huggingAssist](https://github.com/woheller69/huggingassist)** – Provided necessary connections list for Hugging Face
- **[uxwing](https://uxwing.com/)** – Provided the beautiful app icon
- **Kotlin & Material Design 3** – For making development delightful

<br>

## 📄 License

Released under the **GNU General Public License v3.0**.  
See the [full license here](LICENSE).

---

<div align="center">
  <sub>Built with ❤️ by <a href="https://github.com/SilentCoderHere">Silent Coder</a></sub>
</div>
